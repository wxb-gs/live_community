...
import { fetchFollowingNotes } from '../api';
...
export default function FollowingPage() {
  ...
  const loadNotes = useCallback(async (pageNum: number) => {
    ...
    try {
      setError(null);
      const data = await fetchFollowingNotes(pageNum, PAGE_SIZE);
      if (pageNum === 0) {
        setNotes(data);
      } else {
    ...
    <div>
      ...
      {notes.length === 0 && !loading ? (
        <div className="flex flex-col items-center justify-center py-28 text-text-muted">
          ...
          <p className="text-base font-medium text-text-secondary">还没有关注动态</p>
          <p className="text-sm mt-1 text-text-muted">去发现页找到更多有趣的内容和创作者</p>
        </div>
      ) : (
...
function PageHeader() {
  return (
    <div className="sticky top-[61px] z-20 bg-white/80 backdrop-blur-xl border-b border-gray-100/60 px-6 py-3">
      <h2 className="text-[15px] font-bold text-text-primary">关注</h2>
    </div>
  );
}
